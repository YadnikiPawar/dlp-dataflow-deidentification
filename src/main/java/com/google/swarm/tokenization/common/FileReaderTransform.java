...
public abstract class FileReaderTransform
    ...
    public PCollection<KV<String, String>> expand(PBegin input) {
        ...
        return input
                .apply("ReadFileMetadata", PubsubIO.readMessagesWithAttributes().fromSubscription(subscriber()))
    ...
    public class MapPubSubMessage extends DoFn<PubsubMessage, String> {
        private static final String VALID_FILE_PATTERNS = "^(.(?!.*\\.ctl$|.*\\.dlp$|.*\\.xml$|.*\\.json$|.*parallel_composite_uploads.*$|.*\\.schema$))*$";
